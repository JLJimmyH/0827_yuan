...
`matchSpans(A, B, …)` 是兩邊共用的配對規則：**在 r 上有重疊的段就配成一對**
（同一塊材料延續到隔壁射線），配不到的段就地封口——**封口的那一面就是鉛直側壁**。

用「段」而不是「邊界」配是踩過坑才確定的：邊界由內往外配，銑槽對、鑽孔會錯
（把軸心接到外表面，剖面上出現一條橫貫整根棒子的假面）；由外往內配則反過來。
段的重疊關係兩種情形都對。一段最多只配一次（隔壁分岔成兩段時，第二段封口），
這樣每個端點在這一側剛好一條連線，輪廓才串得成封閉圈。

- **剖面 X**：`NC.sim.cylSection(sim, x, src)` → 一組封閉輪廓。內部空洞自成一圈，
  `view2d` 用 `fill('evenodd')` 填出來就是洞。實心軸心那一圈（每段的 lo 都是 0）會被丟掉。
- **3D 成品**：`view3d.buildCylinderMesh()` 把區間的端點當成**分層**：
  第 `lv` 層就是一張 (X, 弧長) 的格網面，偶數層是內向面（lo）、奇數層是外向面（hi）。
  沒有那一層的格就夾到自己最外面的端點，面自然收合、不會破洞。
  只有「四個角至少有一個真的有那一層」的地方才鋪面，否則整根棒子會被好幾張重疊的面蓋住
  （互相 z-fighting，又白白多好幾倍三角形）；四個角都貼在軸心的那種也不鋪（沒有面積）。
  內向面的法線與繞向都要反過來，背面剔除才不會把孔壁剃掉。
  沒切過的圓棒層數 = 1（只有外表面），頂點數與舊版一模一樣。
...
**俯視與剖面 Y** 走 `view2d.cylToCartesian(sim, heightArr)`：把最外層那一圈攤成
...
3. **輪廓的配對規則**。相鄰射線的材料要「配對」才連得成面，配錯的話剖面上會出現
   橫貫整根棒子的假面。**一定要用段的重疊關係配**，不能用邊界的順序配——見上面
   「從區間長回輪廓與網格」。`rotary.test.mjs` 有一條釘住「鑽孔的截面是一條直上直下的槽」。
