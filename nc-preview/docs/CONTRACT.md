...
- **剖面 X**：`NC.sim.cylSection(sim, x, src)` → 一組封閉輪廓。內部空洞若與外界不相通會
  ...
  **封口邊要補轉角點**（`capCorner`）：槽壁與表面的交角幾乎都落在兩條射線之間，
  封口照射線畫的話，槽口的角落會被斜切掉一格——格距 0.5 時是一條 0.5 寬、
  好幾 mm 深的斜口，放大看像被啃了一角（現場問過「為什麼凹的」就是這個）。
  用封口點在另一側的牆點抓走向、外插回表面半徑，角落就閉起來了；
  走向抓不到（孤立封口）或外插跑太遠（t ∉ (1,4)）就退回原本的直線封口。
- **3D 成品**：`view3d.buildCylinderMesh()` 一格四個角，沿 X 與沿周向各配一次，
  ...
  **封口外緣要補轉角點**（`cornerTheta`／`cornerX`，跟剖面的 capCorner 同一招）：
  從隔壁列（欄）半徑最接近的**同型**端點（lo 對 lo、hi 對 hi）抓牆的走向、
  外插回外緣半徑；轉角點**插進**封口的點序（封口面變五、六邊形，扇形三角化），
  不是移動外緣頂點——移走的話封口跟隔壁的表面條帶之間會沿著槽緣開出細縫。
  外插失敗（缺口太淺、抓不到走向、t ∉ (1,4)）就維持原本的直線封口。
