...
### 13.10 層次三：圓棒素材（(X, 弧長) → 半徑）

**不需要體素。** 平面加工用 `(X, Y) → Z` 的高度圖；圓棒加工用 **`(X, 弧長) → 半徑`**：
每一格記「工件表面在這裡離軸心多遠」，初始值就是圓棒半徑，刀切過去就把半徑挖低。
展開之後 `(X, 弧長)` 也是一個平面、半徑就是「高度」，**和平面高度圖完全同構**——
所以 `cutLine` / `cutArc` / `checkRapid` 的演算法大致照用，只多三件事：

1. **兩個方向的格距不同**：周向格距 = 圓周 ÷ 格數（取整之後回頭調格距，繞一圈才會剛好接回原點，
   不會留半格接縫）。所以 sim 有 `cellX` / `cellY`；三軸時兩者都等於 `cell`，行為完全不變。
   舊呼叫端只給 `cell` 的（測試裡自己組 snapshot 物件那種），`cellIndex` 會退回去用它。
2. **周向是循環的**：`wrapY` 為真時 `yRange()` 不把索引夾在 `[0, ny-1]`，讓它超出去，
   由呼叫端取模繞回來；超過一整圈就夾成一圈（再多也是重複蓋同一格）。
3. **刀是直上直下的，不是從軸心射出去的**（見下面「刀底是平面，不是同心圓弧」）。

**刀底是平面，不是同心圓弧。** 展開座標下直接蓋一個「圓盤足跡 + 單一半徑」，等於假設刀是一支
從軸心射出去的楔形刀：足跡寬度變成**弧長**、槽底變成**同心圓弧**。真實的刀是平行機台 Z 的圓柱，
所以 `cutLine` 在圓柱模式（呼叫端多給一個 `cyl = {R, axisS}`，`axisS` 是 A 角換成的弧長）
改成先把刀尖的 `(r, θ)` 換回機台座標的 `(y, z)`（相對迴轉中心），再逐格照機台幾何算：

- **覆蓋**：這一格**表面**點與刀軸的橫向距離（弦長，`height[idx]·sinφ − y刀`）≤ 刀半徑
  → 開口寬度就是刀徑。用當下的 `height` 而不是原始半徑，這樣分層切下去不會多切也不會少切。
- **深度**：這一格的射線打到刀底平面的長度 = `z刀 / cos φ` → 平底。
- **錐尖／球端**的橫向距離要在**刀底那個深度**量（`z刀/cosφ·sinφ − y刀`），
  在表面量的話孔心那格會被多挖一點點，破壞「孔心剛好等於孔底」。

`φ` 只用到 `sin`／`cos`，所以 `s` 與 `A` 差幾圈都無所謂，不必再 unwrap 一次。
`cosφ ≤ 0` 的格（側面、背面）直接跳過：刀底平面照不到。
Ø25 的料用 Ø8 平銑刀切深 5，修正前槽底是 R7.5 的弧、兩側比刀底多挖 0.4 mm，修正後是平的。

**側壁仍然是放射狀的**（越深越窄）。一格只記一個半徑，鉛直側壁在徑向射線上會變成
「表面有料 → 中間空 → 下面又有料」，跟平面高度圖畫不出底切是同一個限制。
要畫對得換資料結構（每格存多段材料區間的 dexel，或每個分度角各一張機台座標的高度圖）。

`cutSegmentCyl` / `checkRapidCyl` 做座標映射：機台 `(x,y,z)` + A → 繞軸心反轉 → `(x, θ, r)`
→ `(x, s = θ·R, r)`，然後照一般的平面蓋章走。映射時要 unwrap 角度，
否則跨過 ±180 的取樣點之間會憑空多一條橫越整圈的切削。
`unrollPoints` 的每個取樣點也帶著當下的 A（`rotarySamples` 是等分內插 `aFrom→a`，照同一個比例還原），
`cutSegmentCyl` 拿它算 `cyl.axisS` 交給 `cutLine`。
...
**3D 成品**：`view3d.buildCylinderMesh()` 把高度圖鋪成圓柱面
（θ = 弧長 ÷ 半徑；y = cy + r·sinθ；z = cz + r·cosθ），周向接回第 0 圈、兩端加封口圓面。
法線用半徑的梯度算（徑向再減掉軸向與周向斜率），孔壁的明暗才不會糊成一片。
有圓柱成品時就不畫圓棒線框了——成品本身就是那根圓棒。
...
2. **刀穿過軸心**。θ 在軸心是未定義的：刀從一側鑽穿過去時 `atan2` 會瞬間跳 180 度、r 從 0 反彈回升，
   ...
   （相鄰點角度跳超過 90 度 → 維持前一點的角度、r 取負）。r < 0 同時也是「鑽穿了」的號誌，
   `cutLine` 據此處理對面那半圈（`iy + ny/2`）——但**只有刀尖真的穿出對面表面**
   （穿透量 ≥ 對面的剩餘半徑）才在對面開口。只是越過軸心一點點的話，對面那段材料還好好的，
   那是個**內部空洞**，高度圖表達不了。早期版本一律照穿透量把對面挖低，等於憑空削掉還在的材料：
   Ø25 的料鑽到越過軸心 1 mm，對面就從 12.5 被削成 1。
   展開圖與模擬共用同一個 `unrollPath`，兩邊不會各走各的。

**仍然做不到**：側凹。跟平面高度圖同一個限制——橫向穿孔的**內壁**、鳩尾槽那種表現不出來
（貫穿孔的兩個開口有，中間的孔道沒有）。對分度鑽孔與軸向銑槽是夠的。
