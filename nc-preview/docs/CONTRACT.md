...
- **K／L 重複時 A 也跟著累加**：`G91 A45. K7` 是「轉 45° 鑽一孔」做 7 次（A45…A315），每個孔各自帶 `aFrom`
  （每孔前面都有一次分度）。只累加 X/Y 會把 7 個孔疊在同一個角度：畫面剩兩個孔、R19 誤報、分度時間少算。
