...
(function (NC) {
  ...
  function checkR37(ctx) {
    ...
    if (rot.mode === 'simultaneous') {
      ...
      out.push(diag('R37', first, 'warning',
        `${A} 軸會把工件轉到 ${rot.angles.length} 個角度加工（${rot.angles.map(deg).join('、')}）——請用「展開圖」看，俯視圖會把各角度疊在同一面上`, {
          detail: '【展開圖】視圖把圓棒表面攤平：橫軸是軸向位置、縱軸是角度。'
            + '分度角度等不等分、孔的軸向位置對不對，這張圖一眼就看得出來。\n\n'
            + '【俯視／剖面／3D】沒有把工件轉過去（那需要體素模型），各角度的加工會疊在同一個面上顯示，不要當成品看。\n\n'
            + '不論看哪一張，這些逐行檢查都是有效的：G 碼語法、模態、刀長／刀徑補正、固定循環參數、'
            + '進給與轉速、換刀順序、每個孔的位置與深度。\n'
            + '不要採信的是：素材殘料、碰撞結果、加工時間——高度圖是 2.5D 的，工件一轉就對不上。',
        }));
    }
        ...
        out.push(diag('R37', first, 'info',
          `刀尖最深切到 Z${f(est.minCutZ)}，已經穿過目前設定的迴轉中心線（Z${f(cz)}）`, {
            detail: '兩種可能，請確認是哪一種：\n'
              ...
              + '到 Project 的「機台」子頁的「第四軸」把迴轉中心 Z 填進去，圖會立刻重畫。',
          }));
      }
    }
